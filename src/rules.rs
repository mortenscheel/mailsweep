...
impl Rules {
    ...
    pub fn get_example_rules() -> Vec<Rule> {
        vec![
            Rule {
                ...
                sender_contains: PatternSet::with_patterns(vec!["newsletter".to_string(), "updates".to_string()]),
            ...
            Rule {
                ...
                subject_contains: PatternSet::with_patterns(vec!["discount".to_string(), "sale".to_string(), "offer".to_string()]),
            ...
            Rule {
                ...
                subject_contains: PatternSet::with_patterns(vec!["tech update".to_string(), "technology news".to_string()]),
                action: RuleAction::Archive,
            }
        ]
    }
    ...
    pub fn load() -> Result<Self> {
        ...
        if rules_path.exists() {
            ...
            match serde_yaml::from_str::<Vec<Rule>>(&rules_str) {
                Ok(rule_items) => {
                    return Ok(Rules { items: rule_items });
                },
                Err(_) => {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let mut content = format!("# yaml-language-server: $schema={}\n\n", schema_path.to_string_lossy());
    ...
    pub fn get_rules_path() -> Result<PathBuf> {
        let rules_dir = Self::get_rules_dir()?;
        Ok(rules_dir.join("rules.yaml"))
    }

    pub fn get_rules_path_str() -> Result<String> {
        Ok(Self::get_rules_path()?.to_string_lossy().to_string())
    }
}
...
mod tests {
    ...
    fn test_yaml_formatting() {
        ...
        let rule = Rule {
            ...
            sender_contains: PatternSet::with_patterns(vec!["test1.com".to_string(), "test2.com".to_string()]),
            subject_contains: PatternSet::with_patterns(vec!["important".to_string(), "urgent".to_string()]),
            action: RuleAction::Archive,
        };
        ...
        let formatted_yaml = yaml.replace(": [", ":\n    - ")
                                 .replace(", ", "\n    - ")
                                 .replace("]", "");
        ...
        assert!(formatted_yaml.contains("sender_contains:\n    - test1.com") || 
    ...
    fn test_rule_validation() {
        ...
        assert!(errors[0].contains("must specify at least one match pattern"), 
        ...
        let errors = rules.validate();
        assert!(errors.is_empty(), "Valid sender rule should pass validation");
        ...
        let errors = rules.validate();
        assert!(errors.is_empty(), "Valid subject rule should pass validation");
        ...
        let errors = rules.validate();
        assert!(!errors.is_empty(), "Rule without name should fail validation");
    ...
    fn test_pattern_set() {
        ...
        assert_eq!(empty.to_vec().len(), 0, "Empty pattern set should have no items");
        
        // Test non-empty pattern set
        let patterns = vec!["a".to_string(), "b".to_string()];
        let non_empty = PatternSet::with_patterns(patterns.clone());
        assert!(!non_empty.is_empty(), "Pattern set with items should not be empty");
        assert_eq!(non_empty.to_vec(), patterns, "Pattern set should return same items");
        
        // Test pattern set with empty strings
        let empty_strings = PatternSet::with_patterns(vec!["".to_string(), "  ".to_string()]);
        assert!(empty_strings.is_empty(), "Pattern set with only empty strings should be considered empty");
    }
}